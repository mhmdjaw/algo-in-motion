import { arrayNumber } from "./SortingVisualizer";

interface animation {
    action: "pivot" | "swap color" | "iterate low" | "iterate high" | "swap values"| "swap pivot" | "swap done" | "swap pivot done";
    index: number[];
}
...
const quickSortHelper = (array: arrayNumber[], low: number, high: number, animations: animation[]): void => {
    if(low < high) {
...
const partition = (array: arrayNumber[], low: number, high: number, animations: animation[]): number => {
    // pivot is array[low]
    let i = low+1;
    let j = high;
    ...
    while(i <= j) { 
        ...
        if(i <= j) {
            ...
            array[j].value = temp;
            // do swap animation
            animations.push({action: "swap values", index: [i, array[i].value, j, array[j].value]});
    ...
    array[j].value = temp;
    // do swap animation
    animations.push({action: "swap values", index: [low, array[low].value, j, array[j].value]});
